...
export class WuhanListingsAdapter implements SourceAdapter {
  ...
  async health(runtime: SourceHealthRuntime) { const startedAt = runtime.now(); const checkedAt = startedAt.toISOString(); const operations = await Promise.all([probeOperationHealth({ operation: searchOperation, router: this.#router, parameters: { url: "https://m.ke.com/wh/ershoufang", query: "property listings", kind: "resale", city: "configured-market", limit: 1 }, runtime }), probeOperationHealth({ operation: detailOperation, router: this.#router, parameters: { url: "https://m.ke.com/wh/ershoufang/104118049193.html", kind: "resale", city: "configured-market" }, runtime })]); return aggregateSourceHealth({ source: manifest.source, displayName: manifest.displayName, checkedAt, durationMs: Math.max(0, runtime.now().getTime() - startedAt.getTime()), operations }); }
}
