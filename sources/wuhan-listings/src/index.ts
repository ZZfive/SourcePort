...
function extract(text: string, kind: ListingKind, city: string, url: string, retrievedAt: string, limit: number): ListingLead[] {
  ...
  for (const match of links) {
    ...
    if (title.length < 4 || /登录|注册|下载|地图|首页|更多|房天下|我的房天下|房地产|APP|切换|买新房|买二手房|查房价|我要卖房|我要出租|找经纪人|找小区|找学校|租房|房贷计算器|装修|商铺|写字楼|土地|数据|资讯|百科|直播|房产问答|房产知识|探房|买房|购房|看房/.test(title)) continue;
    const sourceUrl = new URL(match[1] ?? "", url).toString();
    if (!validUrl(sourceUrl)) continue;
    const nearby = clean(text.slice(Math.max(0, (match.index ?? 0) - 150), Math.min(text.length, (match.index ?? 0) + (match[0]?.length ?? 0) + 350)));
...
export class WuhanListingsAdapter implements SourceAdapter {
  ...
  constructor(options: { fetch?: typeof fetch; openCliCommand?: string } = {}) {
    ...
    this.#router = new BackendRouter([new PublicHttpBackend(backendOptions()), new OpenCliBackend({ name: "wuhan-listings-browser", ...(options.openCliCommand ? { command: options.openCliCommand } : {}), jsonOutput: true, args: ({ request }) => ["web", "read", "--url", String((request.parameters as Record<string, unknown>)["url"]), "--stdout", "true", "--download-images", "false", "--wait", "1"], parse: (data, context) => { const p = context.request.parameters as Record<string, unknown>; const value = typeof data === "string" ? data : data && typeof data === "object" ? String((data as Record<string, unknown>)["content"] ?? (data as Record<string, unknown>)["markdown"] ?? "") : ""; const city = String(p["city"]); const url = String(p["url"]); const retrievedAt = new Date().toISOString(); return context.request.operation === searchOperation.operation ? parseSearch(`<article>${value}</article>`, String(p["query"]), p["kind"] as ListingKind, city, url, retrievedAt, Number(p["limit"] ?? 20)) : parseDetail(`<article>${value}</article>`, p["kind"] as ListingKind, city, url, retrievedAt); } }), new ManualStepBackend({ name: "wuhan-listings-manual", description: "Open the listing search page in a browser and retry" })]);
  }
...
export const __test__ = { extract, validUrl, parseSearch, parseDetail, searchOperation, detailOperation };
