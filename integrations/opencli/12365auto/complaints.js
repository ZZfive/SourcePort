...
cli({ site:'12365auto', name:'complaints', description:'Search public 12365auto complaint listings (read-only)', access:'read', domain:'12365auto', strategy:Strategy.PUBLIC, browser:false, args:[{name:'query', positional:true, required:true, help:'Brand or series keyword'},{name:'limit',type:'int',default:20,help:'Maximum rows (1-50)'}], columns:['complaintId','brand','series','model','summary','submittedAt','status','url'], func:async ({query,limit=20}) => { const body=await html(`${BASE}/zlts/`); const out=rows(body, query).filter(x => !query || `${x.summary} ${x.series}`.includes(query)).slice(0, Math.min(Number(limit)||20,50)); if(!out.length) throw new Error('12365auto returned no usable complaints'); return out; } });
