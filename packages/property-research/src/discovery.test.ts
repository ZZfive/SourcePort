...
describe("property discovery route enrichment", () => {
  ...
  it("attaches route evidence to the candidate discovered earlier in the request list", async () => {
    const result = await discoverPropertyCandidates([
      ...
      execute: async (request) => request.operation === "search-listings"
        ? { requestId: "listing", source: request.source, operation: request.operation, operationSchemaVersion: "1.0.0", status: "success", data: { query: "示例区域", kind: "new", items: [{ candidateId: "lead-1", kind: "new", city: "示例城市", community: "示例小区", title: "示例小区", sourceUrl: "https://listing.example/lead-1", retrievedAt: "2026-09-18T00:00:00Z", priceCny: { minimumCny: 1500000, maximumCny: 1500000 }, evidenceStatus: "lead-only" }] }, evidence: [{ id: "listing-evidence", source: "listings", operation: request.operation, backend: "fixture", retrievedAt: "2026-09-18T00:00:00Z", verification: "source-verified" }], warnings: [], recoveryActions: [] }
    ...
    expect(result.candidates[0]?.askingPrice).toEqual({ minimumCny: 1500000, maximumCny: 1500000 });
    expect(result.candidates[0]?.purchasePrice).toBeUndefined();
  ...
  it("enriches a candidate from a claimed mini-program observation and keeps provenance", async () => {
    ...
    expect(result.candidates[0]).toEqual(expect.objectContaining({ areaSqm: 92, bedrooms: 3, livingRooms: 1, askingPrice: { minimumCny: 1500000, maximumCny: 1500000 } }));
    expect(result.candidates[0]?.purchasePrice).toBeUndefined();
